    ...
    userQueries.createUser(newUser, (err, user) => {
      if (err) {
        ...
        passport.authenticate("local")(req, res, () => {
          req.flash(
            "notice",
            "Welcome to WikiEats! Start by creating a new Wiki."
          );
          res.redirect("/");
        });
      }
        ...
        if (err && err.type === "StripeCardError") {
          ...
          userQueries.upgradeUser(req, (err, user) => {
            if (err || user == null) {
              ...
              req.flash(
                "notice",
                "Your account has been upgraded! You will receive a confirmation email shortly."
              );
        ...
        wikiQueries.downgradeWikis(user.id, (err, wiki) => {
          if (err) {
            ...
            if (wiki == null) {
              ...
              collaboratorQueries.removeCollaborators(
                wiki,
                (err, collaborator) => {
                  if (err) {
                    console.log(err);
